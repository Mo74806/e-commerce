...
searchBtn.addEventListener("click", function () {
  setCookie("category", document.querySelector("#search-category").value, 1);
  categoryAllProducts(
...
function categoryAllProducts(categoryName, number, selectedArea) {
  ...
  if (number == 0) number = categoryProducts.length;
  ...
  for (var i = 0; i < productsCart.length; i++) {
    productsCart[i].addEventListener("click", function (e) {
      ...
      document.querySelector(".cart-list").classList.add("d-none"); //make the container visable
...
categoryAllProducts(category, 0, ".selected-category");
localStorage.setItem("cart", JSON.stringify(cart));
